...
function parseSkuConfigRows(headers, rows) {
  ...
  rows.forEach((row) => {
    ...
    entries.push({ sku, type, name: productName, cogs });
  });
...
function computeOrderMetrics(order, settings) {
  const isLoss = order.status === STATUS.CANCELLED || order.status === STATUS.RETURNED;
  const nettRevenue = isLoss ? 0 : (order.settlementAmount > 0 ? order.settlementAmount : order.gmv);
  const packagingFee = settings.packagingFee; // áp dụng cho cả đơn thành công & đơn hoàn (ghi nhận tổn thất)
  const profitBeforeAds = nettRevenue - order.cogsTotal - order.feesTotal - packagingFee;
  return { ...order, nettRevenue, packagingFee, profitBeforeAds };
}
...
const initialState = {
  ...
  logs: [
    { id: uid("log"), time: new Date().toLocaleString("vi-VN"), type: "success", message: "Đã nạp dữ liệu thật TikTok Shop từ Seller Center: 23 đơn hàng (25-26/08/2026) và 166 SKU sản phẩm." },
    { id: uid("log"), time: new Date().toLocaleString("vi-VN"), type: "warning", message: "166 SKU đang có Giá vốn = 0đ (Seller Center không xuất giá vốn) — vào tab Config Center, bấm vào ô 'Giá vốn' để nhập tay." },
    { id: uid("log"), time: new Date().toLocaleString("vi-VN"), type: "warning", message: "File 'Tất cả đơn hàng' không có cột phí sàn chi tiết (Transaction fee/Commission...) nên Tổng phí sàn = 0đ — import thêm file Đối soát/Statement of Account để tính đủ." },
    { id: uid("log"), time: new Date().toLocaleString("vi-VN"), type: "info", message: "File 'Đơn trả hàng hoàn tiền' (42 dòng) thuộc khoảng thời gian khác với 23 đơn vừa nhập nên chưa khớp đơn nào — hệ thống sẽ tự khớp khi bạn import thêm lịch sử đơn hàng cũ hơn." },
    { id: uid("log"), time: new Date().toLocaleString("vi-VN"), type: "info", message: "Shopee chưa có dữ liệu — import file đơn hàng Shopee ở tab Import Center khi sẵn sàng." },
  ],
  dateRange: { start: "", end: "" },
};
  ...
  switch (action.type) {
    ...
    case "IMPORT_SKU_CONFIG": {
      ...
      const log = {
        ...
        time: new Date().toLocaleString("vi-VN"),
        type: "success",
        message: `[${platformLabel(platform)}] Cập nhật Config SKU "${fileName}": ${entries.length} SKU.`,
      };
      return { ...state, skuConfig: { ...state.skuConfig, [platform]: entries }, logs: [log, ...state.logs].slice(0, 50) };
    }
    ...
    case "RESTORE_INITIAL_IMPORT":
      ...
      return {
        ...
        logs: [{ id: uid("log"), time: new Date().toLocaleString("vi-VN"), type: "info", message: "Đã khôi phục snapshot dữ liệu TikTok Shop ban đầu (23 đơn, 166 SKU)." }, ...state.logs].slice(0, 50),
      };
    default:
      return state;
  }
    ...
    <div className="bg-white border-b border-slate-200 sticky top-0 z-30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex flex-wrap items-center gap-3 justify-between">
        <div className="flex items-center gap-2">
          <div className="w-9 h-9 rounded-xl bg-blue-950 flex items-center justify-center text-white font-bold text-sm">EE</div>
          <div>
            <div className="font-semibold text-blue-950 leading-tight">ELLA Eyewear</div>
            <div className="text-xs text-slate-400 leading-tight">Profit Control Center</div>
          </div>
        </div>
      ...
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 flex gap-1 overflow-x-auto pb-1">
        {[
          { key: "dashboard", label: "Dashboard tổng hợp", icon: LayoutDashboard },
          { key: "tiktok", label: "TikTok Shop chi tiết", icon: Store },
          { key: "shopee", label: "Shopee chi tiết", icon: ShoppingBag },
          { key: "import", label: "Import Center", icon: FileSpreadsheet },
          { key: "config", label: "Config Center", icon: Settings },
        ].map((t) => (
          <button
            key={t.key}
            onClick={() => setActiveTab(t.key)}
            className={`flex items-center gap-1.5 px-3.5 py-2 text-sm font-medium border-b-2 whitespace-nowrap transition ${
              activeTab === t.key
                ? "border-pink-600 text-blue-950"
...
function SkuConfigTable({ platform }) {
  const { state } = useApp();
  const [search, setSearch] = useState("");
  ...
  const typeLabel = { kinh: "Kính mắt", hop: "Hộp", khan: "Khăn lau", khac: "Khác" };
  const typeTone = { kinh: "navy", hop: "pink", khan: "amber", khac: "gray" };
  ...
  return (
    <Card className="p-4">
      ...
      {missingCogsCount > 0 && (
        <div className="flex items-start gap-1.5 text-[11px] text-amber-700 bg-amber-50 rounded-lg px-2.5 py-2 mb-2">
          <AlertCircle size={12} className="shrink-0 mt-0.5" />
          {missingCogsCount} SKU đang có giá vốn = 0đ — bấm trực tiếp vào ô giá vốn bên dưới để nhập.
        </div>
      )}
      <div className="overflow-x-auto max-h-72 overflow-y-auto">
        <table className="w-full text-xs min-w-[420px]">
          ...
          <tbody>
            {list.map((e, i) => (
              <tr key={e.sku + i} className="border-b border-slate-50 last:border-0">
                <td className="py-2 pr-2 font-mono text-blue-950">{e.sku}</td>
                <td className="py-2 pr-2"><Badge tone={typeTone[e.type]}>{typeLabel[e.type]}</Badge></td>
...
function ConfigCenterTab() {
  ...
  return (
    <div className="space-y-5">
      <RolePanel />
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <SkuConfigTable platform={PLATFORM.TIKTOK} />
        <SkuConfigTable platform={PLATFORM.SHOPEE} />
      </div>
      <SettingsForm />
      <Card className="p-4">
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-5">
        ...
        {activeTab === "tiktok" && <PlatformDetailTab platform={PLATFORM.TIKTOK} />}
        {activeTab === "shopee" && <PlatformDetailTab platform={PLATFORM.SHOPEE} />}
        {activeTab === "import" && <ImportCenterTab />}
        {activeTab === "config" && <ConfigCenterTab />}
      </main>
      <footer className="text-center text-[11px] text-slate-300 py-4">
        ELLA Eyewear · Profit Control Center · Dữ liệu xử lý cục bộ trên trình duyệt, không lưu trữ máy chủ ngoài.
      </footer>
    </div>
  );
}
